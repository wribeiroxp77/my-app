...
export function useXPHistoryLast7Days(): XPHistoryPoint[] {
  const [data, setData] = React.useState<XPHistoryPoint[]>([]);

  React.useEffect(() => {
    const update = () => {
      const all = readHistoricoTarefas();
      const today = new Date();
      const result: XPHistoryPoint[] = [];
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        result.push({ date: dateStr, xp: entry?.xp ?? 0 });
      }
    ...
    const handleStorage = (event: StorageEvent) => {
      if (!event.key || event.key === "historico_tarefas") {
        update();
      }
    };
    ...
    window.addEventListener("historico_tarefas_updated", handleCustom);

    return () => {
      window.removeEventListener("storage", handleStorage);
      window.removeEventListener("historico_tarefas_updated", handleCustom);
    };
  }, []);
