...
function StatsTab({ streak, tasks, dayStats, language }: {
  streak: number;
  tasks: Task[];
  dayStats: DayStats[];
  language: "en" | "pt";
  ...
  const mediumCount = completedTasks.filter((t) => t.difficulty === "medium").length;
  ...
  // ── Badges ────────────────────────────────────────────────────────────────
  const badges = [
    { icon: "⭐", label: "100 Tasks",   current: completedTasks.length, goal: 100 },
...
<p className="text-purple-300/70 text-sm mt-1">Your journey through the stars</p>
      </motion.div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {[
          ...
          <motion.div
            ...
            className={`rounded-2xl border ${card.border} bg-gradient-to-br ${card.color} backdrop-blur-xl p-4 flex flex-col items-center justify-center gap-1 shadow-[0_0_20px_rgba(168,85,247,0.08)]`}
          >
            <span className="text-2xl">{card.icon}</span>
            <span className={`text-2xl font-bold ${card.text}`}>{card.value}</span>
            <span className="text-white/50 text-xs text-center">{card.label}</span>
          </motion.div>
        ))}
      </div>
      ...
      >
        ...
        <div className="flex flex-col gap-3">
          {[
            { label: "Easy",   count: easyCount,   color: "#06b6d4", bg: "bg-cyan-500",   pct: Math.round((easyCount   / totalDone) * 100) },
            { label: "Medium", count: mediumCount, color: "#8b5cf6", bg: "bg-purple-500", pct: Math.round((mediumCount / totalDone) * 100) },
            { label: "Hard",   count: hardCount,   color: "#f59e0b", bg: "bg-amber-500",  pct: Math.round((hardCount   / totalDone) * 100) },
          ].map((row, i) => (
            <div key={row.label}>
      ...
      <motion.div
        ...
        className="rounded-2xl border border-purple-500/40 bg-gradient-to-br from-black/40 via-purple-900/10 to-black/40 backdrop-blur-xl p-5 shadow-[0_0_40px_rgba(168,85,247,0.08)]"
      >
        <h3 className="text-white font-semibold mb-4 text-sm tracking-wide uppercase opacity-80">
          🏅 Achievements
        </h3>
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-2">
                    <span className={`text-xl ${unlocked ? "" : "grayscale opacity-50"}`}>
                      {badge.icon}
                    </span>
                    <span className={`text-sm font-medium ${unlocked ? "text-white" : "text-white/50"}`}>
                      {badge.label}
                    </span>
                    {unlocked && (
                ...
                <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${pct}%` }}
                    transition={{ duration: 0.8, delay: 0.45 + i * 0.07, ease: "easeOut" }}
                    className="h-full rounded-full"
                    style={{
                      background: unlocked
                        ? "linear-gradient(90deg, #a78bfa, #818cf8)"
                        : "linear-gradient(90deg, #6d28d9, #4c1d95)",
                      boxShadow: unlocked ? "0 0 8px rgba(167,139,250,0.6)" : "none",
                    }}
                  />
                </div>
              </motion.div>
...
function ProfileTab({
  streak, tasks, setTasks, dayStats, setDayStats, weeklyTasks, setWeeklyTasks,
  setStreak, showSettings, setShowSettings, showWeeklyEditor, setShowWeeklyEditor,
  editingDay, setEditingDay, playerName, setPlayerName, editingName, setEditingName,
  tempName, setTempName, language, setLanguage, resetModal, setResetModal,
  exportData, editingWeeklyTaskId, setEditingWeeklyTaskId, editingWeeklyText,
  setEditingWeeklyText, startEditingWeeklyTask, saveEditWeeklyTask,
  cancelEditWeeklyTask, addTaskToDay, deleteWeeklyTask,
}: {
  streak: number;
  tasks: Task[];
  ...
  const T = {
    en: {
      profile: "Profile", subtitle: "Your command center",
      editTasks: "Edit Tasks", settings: "Settings",
      editWeekly: "Edit Weekly Tasks", schedule: "Customize your schedule",
      back: "← Back", addTask: "Add task",
      settingsTitle: "Settings", settingsSubtitle: "App configuration",
      resetToday: "Reset Today", resetAll: "Reset Everything",
      export: "Export Data", language: "Language",
      ...
      cancel: "Cancel", confirm: "Confirm", tasks: "Tasks", level: "Level",
      tapToEdit: "Tap to edit name",
    },
    pt: {
      profile: "Perfil", subtitle: "Seu centro de comando",
      editTasks: "Editar Tasks", settings: "Configurações",
      editWeekly: "Editar Tasks Semanais", schedule: "Personalize sua agenda",
      back: "← Voltar", addTask: "Adicionar task",
      settingsTitle: "Configurações", settingsSubtitle: "Configuração do app",
      resetToday: "Resetar Hoje", resetAll: "Resetar Tudo",
      export: "Exportar Dados", language: "Idioma",
      ...
      cancel: "Cancelar", confirm: "Confirmar", tasks: "Tasks", level: "Nível",
  ...
  let level = 1;
  let xpLeft = totalXP;
  while (xpLeft >= 50 * level) { xpLeft -= 50 * level; level++; }
  const xpNeeded = 50 * level;
  const xpPct = Math.min((xpLeft / xpNeeded) * 100, 100);

  const hardCount = completedTasks.filter((tk) => tk.difficulty === "hard").length;
...
const getPlayerTitle = () => {
  ...
  if (completedTasks.length >= 20)       return { icon: "⚡", label: "Rising Star" };
...
const handleNotification = async () => {
        ...
        >
          ...
          >
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={resetModal === "today" ? handleResetToday : handleResetAll}
      ...
      {!showSettings && !showWeeklyEditor && (
        <>
          ...
          <motion.div
            ...
            className="rounded-2xl border border-purple-500/40 bg-gradient-to-br from-black/40 via-purple-900/20 to-black/40 backdrop-blur-xl p-6 mb-4 shadow-[0_0_40px_rgba(168,85,247,0.12)]"
          >
            {/* Avatar */}
  ...
  <img src="/logo.png" alt="Single Player" className="w-full h-full object-cover" />
</div>
              ...
              {editingName ? (
                ...
                >
                  ...
                  <span className="text-white/30 text-xs group-hover:text-purple-400 transition-colors">✏️</span>
                </button>
              )}
              ...
              <div className="mt-2 flex items-center gap-1.5 bg-purple-500/20 border border-purple-500/40 rounded-full px-3 py-1">
                ...
                <span className="text-purple-300 text-xs font-semibold tracking-wide">{title.label}</span>
              </div>
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
              {[
                { label: t.level, value: level,                 color: "text-purple-400", icon: "⚡" },
                { label: "XP",    value: totalXP,               color: "text-cyan-400",   icon: "💎" },
                { label: t.tasks, value: completedTasks.length, color: "text-blue-400",   icon: "✅" },
              ].map((stat) => (
                <div
                  key={stat.label}
          ...
          {/* Botões */}
          {[
            { label: t.editTasks, icon: "📅", delay: 0.1, onClick: () => setShowWeeklyEditor(true), border: "border-blue-500/40", text: "text-blue-300", glow: "hover:shadow-[0_0_20px_rgba(59,130,246,0.15)]" },
           { label: t.settings, icon: "⚙️", delay: 0.15, onClick: () => setShowSettings(true), border: "border-purple-500/40", text: "text-purple-300", glow: "hover:shadow-[0_0_20px_rgba(139,92,246,0.15)]" },
].map((btn) => (
            <motion.button
              key={btn.label}
...
>
  <div className="flex items-center gap-3">
    ...
    <span className="font-semibold text-blue-300">Daily Reminder</span>
  </div>
  <span className="text-white/30">›</span>
</motion.button>
...
>
  <p className="text-white/60 text-sm leading-relaxed">
    "You are the protagonist of this game.<br />Level up your life."
  </p>
      ...
      {showWeeklyEditor && (
        <>
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.4, delay: 0.05 }}
            className="rounded-2xl border border-purple-500/40 bg-gradient-to-br from-black/40 via-purple-900/10 to-black/40 backdrop-blur-xl p-4 mb-4 shadow-[0_0_40px_rgba(168,85,247,0.08)]"
          >
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <h4 className="font-bold text-white text-sm">
                    {DAY_NAMES[language][editingDay as keyof typeof DAY_NAMES["en"]]}
                  </h4>
                  <button onClick={() => setEditingDay(null)} className="text-white/40 hover:text-white transition-colors">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                  ...
                  {(weeklyTasks[editingDay] || []).map((task) => (
                    ...
                    >
                      {editingWeeklyTaskId === task.id ? (
                        ...
                        <>
                          <span className="flex-1 text-white/80 text-sm">{task.text}</span>
                          <button
      ...
      {showSettings && (
        <>
          ...
          >
            ...
            <h1 className="text-2xl font-bold text-white">⚙️ {t.settingsTitle}</h1>
            <p className="text-purple-300/70 text-sm mt-1">{t.settingsSubtitle}</p>
          </motion.div>
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              ...
              >
                <span className={`text-sm font-bold transition-colors ${language === "en" ? "text-cyan-300" : "text-white/30"}`}>EN</span>
                <span className="text-white/20 mx-1">|</span>
                <span className={`text-sm font-bold transition-colors ${language === "pt" ? "text-cyan-300" : "text-white/30"}`}>PT</span>
              </button>
            </motion.div>
            ...
            <motion.button
              ...
              className="w-full rounded-2xl border border-amber-500/40 bg-gradient-to-br from-black/40 via-amber-900/10 to-black/40 backdrop-blur-xl p-4 flex items-center justify-between hover:shadow-[0_0_20px_rgba(245,158,11,0.12)] transition-all duration-300"
            >
              <div className="flex items-center gap-3">
                <span className="text-xl">⚠️</span>
                <span className="font-semibold text-amber-400">{t.resetToday}</span>
              </div>
              <span className="text-white/30">›</span>
            </motion.button>
            ...
            <motion.button
              ...
              className="w-full rounded-2xl border border-red-500/40 bg-gradient-to-br from-black/40 via-red-900/10 to-black/40 backdrop-blur-xl p-4 flex items-center justify-between hover:shadow-[0_0_20px_rgba(239,68,68,0.12)] transition-all duration-300"
            >
              <div className="flex items-center gap-3">
                <span className="text-xl">💀</span>
            ...
            <motion.button
              ...
              transition={{ duration: 0.35, delay: 0.2 }}
              onClick={exportData}
              className="w-full rounded-2xl border border-purple-500/40 bg-gradient-to-br from-black/40 via-purple-900/10 to-black/40 backdrop-blur-xl p-4 flex items-center justify-between hover:shadow-[0_0_20px_rgba(139,92,246,0.12)] transition-all duration-300"
            >
              <div className="flex items-center gap-3">
                <Download className="w-5 h-5 text-purple-400" />
                <span className="font-semibold text-purple-300">{t.export}</span>
              </div>
              <span className="text-white/30">›</span>
            </motion.button>
...
const [playerName, setPlayerName] = useState<string>(() => localStorage.getItem("playerName") || "Player");
const [editingName, setEditingName] = useState(false);
const [tempName, setTempName] = useState("");
const [language, setLanguage] = useState<"en" | "pt">(() => (localStorage.getItem("language") as "en" | "pt") || "en");
...
>
 ...
 {showDifficultyMenu && (
      ...
      <div className="flex gap-3">
        ...
        {currentTab === "stats" && (
          <AnimatedPage key="stats" className="w-full max-w-md">
           <StatsTab streak={streak} tasks={tasks} dayStats={dayStats} language={language} />
          </AnimatedPage>
        )}
  ...
  <AnimatedPage key="profile" className="w-full max-w-md">
    <ProfileTab
      streak={streak} tasks={tasks} setTasks={setTasks}
      dayStats={dayStats} setDayStats={setDayStats}
      weeklyTasks={weeklyTasks} setWeeklyTasks={setWeeklyTasks}
      setStreak={setStreak} showSettings={showSettings}
      setShowSettings={setShowSettings} showWeeklyEditor={showWeeklyEditor}
      setShowWeeklyEditor={setShowWeeklyEditor} editingDay={editingDay}
      setEditingDay={setEditingDay} playerName={playerName}
      setPlayerName={setPlayerName} editingName={editingName}
      setEditingName={setEditingName} tempName={tempName}
      setTempName={setTempName} language={language}
      setLanguage={setLanguage} resetModal={resetModal}
      setResetModal={setResetModal} exportData={exportData}
  ...
  <div className="mx-3 mb-3 rounded-2xl border border-purple-500/40 bg-black/60 backdrop-blur-xl shadow-[0_0_30px_rgba(139,92,246,0.15)]">
    <div className="max-w-md mx-auto px-2 py-2 flex justify-around items-center relative">
        ...
        >
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
            <motion.path
              ...
              fill={currentTab === "home" ? "rgba(168,85,247,0.15)" : "none"}
            />
            {currentTab === "home" && (
              <motion.path
                d="M3 12L12 3L21 12V21H15V15H9V21H3V12Z"
      ...
      >
        ...
        >
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
            <motion.rect x="3" y="12" width="4" height="9" rx="1"
              stroke={currentTab === "stats" ? "#06b6d4" : "#164e63"}
              strokeWidth="1.5"
              fill={currentTab === "stats" ? "rgba(6,182,212,0.15)" : "none"}
            />
            <motion.rect x="10" y="7" width="4" height="14" rx="1"
              stroke={currentTab === "stats" ? "#06b6d4" : "#164e63"}
              strokeWidth="1.5"
              fill={currentTab === "stats" ? "rgba(6,182,212,0.15)" : "none"}
            />
            <motion.rect x="17" y="3" width="4" height="18" rx="1"
              stroke={currentTab === "stats" ? "#06b6d4" : "#164e63"}
              strokeWidth="1.5"
              fill={currentTab === "stats" ? "rgba(6,182,212,0.15)" : "none"}
            />
        ...
        >
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
            <motion.circle cx="12" cy="8" r="4"
              ...
              strokeWidth="1.5"
              fill={currentTab === "profile" ? "rgba(59,130,246,0.15)" : "none"}
            />
            <motion.path
